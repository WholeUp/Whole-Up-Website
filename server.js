...
// Global canonical URL middleware
app.use((req, res, next) => {
  const host = req.get('host');
  // Always resolve canonical to HTTPS version for production SEO
  res.locals.canonicalUrl = `https://${host}${req.originalUrl.split('?')[0]}`;
  next();
});
...
// DYNAMIC SITEMAP (Advanced SEO)
app.get('/sitemap.xml', (req, res) => {
  const baseUrl = 'https://www.wholeup.in';
